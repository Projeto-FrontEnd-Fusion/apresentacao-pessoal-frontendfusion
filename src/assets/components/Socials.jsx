export const Socials = () => {
  return(
    <article className="text-lightgray flex gap-4 justify-center lg:justify-start">

      <a href="https://github.com/Leticiafbm" target="_blank" className="hover:text-lightcyan active:text-lightcyan">
        <svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 
          3.438 9.8 8.205 11.387.6.113.82-.258.82-.577 
          0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.387-1.333-1.757-1.333-1.757-1.09-.745.084-.729.084-.729 
          1.205.084 1.84 1.236 1.84 1.236 1.07 1.835 2.807 1.304 3.495.997.108-.776.418-1.305.762-1.605-2.665-.3-5.467-1.332-5.467-5.93 
          0-1.31.467-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 
          0 0 1.005-.322 3.3 1.23a11.52 11.52 0 0 1 3-.405c1.02.005 
          2.045.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 
          1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 
          5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 
          0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297 
          c0-6.627-5.373-12-12-12"/>
        </svg>
      </a>
      
      <a href="https://www.linkedin.com/in/let%C3%ADcia-brito-65186b346/" target="_blank" className="hover:text-lightcyan active:text-lightcyan">
       <svg xmlns="http://www.w3.org/2000/svg" width="34" height="34" fill="currentColor" viewBox="0 0 24 24">
          <path d="M20.447 20.452H17.2V14.8c0-1.345-.027-3.077-1.88-3.077-1.88 0-2.17 
          1.466-2.17 2.977v5.752H9V9h3.1v1.561h.045c.433-.82 
          1.494-1.683 3.075-1.683 3.29 0 3.894 2.165 3.894 
          4.981v6.593zM5.337 7.433a1.86 1.86 0 1 1 
          0-3.72 1.86 1.86 0 0 1 0 3.72zM6.918 20.452H3.75V9h3.168v11.452zM22.225 
          0H1.771C.792 0 0 .774 0 1.723v20.554C0 23.227.792 24 1.771 
          24h20.451C23.2 24 24 23.227 24 22.277V1.723C24 
          .774 23.2 0 22.222 0z"/>
        </svg>
      </a>
    </article>
  );
}